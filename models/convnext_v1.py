...
class ConvNeXtV1(nn.Module):
    def __init__(
        ...
        in_chans=3, 
        num_classes=1000, 
        depths=[2, 2, 6, 2], # ConvNeXt V1 Atto
        dims=[40, 80, 160, 320], # ConvNeXt V1 Atto
        drop_path_rate=0.0, 
        ...
        self.stages = nn.ModuleList() # 이거
        dp_rates = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]
        cur = 0

        for i in range(4):
            stage = nn.Sequential(
                *[V1Block(dim=dims[i], drop_path=dp_rates[cur + j], layer_scale_init_value=layer_scale_init_value) 
